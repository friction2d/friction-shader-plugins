...
vec3 ring(vec2 uv, vec2 pos, float radius, float thick)
{
    ...
    vec3 f0 = vec3(0.0);
    f0 += mix(0.0, 1.0, smoothstep(thick, thick-softness, abs(length(pos+uv) - radius*1.0)))*Color1.rgb;
    f0 += mix(0.0, 1.0, smoothstep(thick, thick-softness, abs(length(pos+uv) - radius*1.1)))*Color2.rgb;
    f0 += mix(0.0, 1.0, smoothstep(thick, thick-softness, abs(length(pos+uv) - radius*1.2)))*Color3.rgb;

...
uniform vec2 MOVEMENT_DIRECTION;// = vec2(0.7, -1.0);

uniform float PARTICLE_SIZE;// =0.005

uniform vec2 PARTICLE_SCALE;// =(vec2(0.5, 1.6));
uniform vec2 PARTICLE_SCALE_VAR;//= (vec2(0.25, 0.2));

uniform vec2 PARTICLE_BLOOM_SCALE;//= (vec2(0.5, 0.8));
uniform vec2 PARTICLE_BLOOM_SCALE_VAR;// =(vec2(0.3, 0.1));

uniform vec4 SPARK_COLOR;// = vec4(1.0, 0.4, 0.05,1.0) * 1.5;
uniform vec4 BLOOM_COLOR;// =vec4(1.0, 0.4, 0.05,1.0)* 0.8;
uniform vec4 SMOKE_COLOR;// = vec4(1.0, 0.43, 0.1,1.0) * 0.8;

uniform float SIZE_MOD ;//= 1.05;
uniform float ALPHA_MOD;//= 0.9;
uniform int LAYERS_COUNT;// =15;
...
vec3 fireParticles(in vec2 uv, in vec2 originalUV)
{
    ...
    //Add sparks
    particles += (1.0 - smoothstep(PARTICLE_SIZE * 0.6, PARTICLE_SIZE * 3.0, dist)) * SPARK_COLOR.rgb;
    
    //Add bloom
    particles += pow((1.0 - smoothstep(0.0, PARTICLE_SIZE * 6.0, distBloom)) * 1.0, 3.0) * BLOOM_COLOR.rgb;
...
void main(void)
{
   ...
   //  vec2 uv = gl_FragCoord.xy /resolution.xy;
    ...
    vec3 smoke = smokeIntensity * SMOKE_COLOR.rgb * 0.8 * vignette;
   ...
   vec3 col = particles + smoke + SMOKE_COLOR.rgb * 0.02;
	col *= vignette;
